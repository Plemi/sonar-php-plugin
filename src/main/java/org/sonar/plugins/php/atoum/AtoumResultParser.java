...
import org.sonar.plugins.php.atoum.xml.TestCase;
import org.sonar.plugins.php.atoum.xml.TestSuite;
import org.sonar.plugins.php.atoum.xml.TestSuites;

import com.thoughtworks.xstream.XStream;

/**
 * The Class AtoumResultParser.
 */
public class AtoumResultParser implements BatchExtension {
  ...
  /** The logger. */
  private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(AtoumResultParser.class);
   ...
   *          the context
   */
  public AtoumResultParser(Project project, SensorContext context) {
    super();
  ...
  protected TestSuites getTestSuites(File report) {
    ...
    try {
      ...
      throw new SonarException("Can't read Atoum report : " + report.getAbsolutePath(), e);
    } finally {
   ...
   */
  private Resource<?> getUnitTestResource(AtoumTestReport report) {
    LOG.info("getUnitTestResource: " + report.getFile());
    LOG.info("getUnitTestResource: " + project.getFileSystem().getTestDirs().toString());
  ...
  private void parseFile(File report) {
    ...
    List<AtoumTestReport> fileReports = readSuites(testSuites);
    StringBuilder details = new StringBuilder();
    for (AtoumTestReport fileReport : fileReports) {
      LOG.debug("parseFile: " + fileReport.toString());
      
      StringBuilder fileDetails = saveTestReportMeasures(fileReport);
      details.append(fileDetails.toString());
      
      Resource<?> unitTestResource = getUnitTestResource(fileReport);    
      if (unitTestResource != null) {    	    	    	
    	StringBuilder fileMeasure = new StringBuilder();
    	fileMeasure.append("<tests-details>");
    	fileMeasure.append(fileDetails);
    	fileMeasure.append("</tests-details>");
    	LOG.info(unitTestResource.toString());
    	LOG.info("parseFile fileDetails : " + fileMeasure.toString());
    	
        context.saveMeasure(unitTestResource, new Measure(CoreMetrics.TEST_DATA, fileMeasure.toString()));
      }
    }
    
    saveTestsDetails(details);
  }
   ...
   * @param testSuites
   *          the test suites
   * @return List<AtoumTestReport> A list of all test reports
   */
  private List<AtoumTestReport> readSuites(TestSuites testSuites) {
    List<AtoumTestReport> result = new ArrayList<AtoumTestReport>();
   ...
   *          the unit test report
   */
  protected StringBuilder saveTestReportMeasures(AtoumTestReport fileReport) {
    if (!fileReport.isValid()) {
      return null;
    }
    LOG.debug(fileReport.toString());
    Resource<?> unitTestResource = getUnitTestResource(fileReport);
    if (unitTestResource != null) {// fileReport.getTests() > 0) {
      ...
      return makeTestDetails(fileReport);
    } else {
  ...
  private StringBuilder makeTestDetails(AtoumTestReport fileReport) {
	  StringBuilder details = new StringBuilder();
   ...
   *          the file report
   */
  private void saveTestsDetails(StringBuilder report) {
    StringBuilder details = new StringBuilder();
    details.append("<tests-details>");
    details.append(report.toString());
    details.append("</tests-details>");
