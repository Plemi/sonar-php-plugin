...
public class PhpTestSuiteReader {
  ...
  */
  ...
  private Map<String, AtoumTestReport> reportsPerClass = new HashMap<String, AtoumTestReport>();
   ...
   */
  private void cumulateTestCaseDetails(TestCase testCase, AtoumTestReport report) {
   ...
   * 
   * Due to a inconsistent XML format in atoum, we have to parse enclosing testsuite name for generated testcases when a testcase holds
   * the annotation dataProvider.
   * 
   * @param testSuite
   *          the test suite
   * @return List<AtoumTestReport> A list containing on report per php class
   */
  public void readSuite(TestSuite testSuite, String parentFileName) {
    ...
    if (testCases != null) {
      for (TestCase testCase : testCases) {
        ...
        AtoumTestReport report = reportsPerClass.get(testClassName);
        // If no reports exists for this class we create one
        if (report == null) {
          report = new AtoumTestReport();
   ...
   * 
   * @return the collection of {@link AtoumTestReport} objects
   */
  public Collection<AtoumTestReport> getReportsPerClass() {
